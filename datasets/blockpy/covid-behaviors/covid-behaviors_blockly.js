...
var covid-behaviors_INDEX_VALUES = {
    "(None)": [],
...
var covid-behaviors_PROPERTIES = [
    ["Country", "Country"] ,
    ["Days since outbreak", "Days since outbreak"] ,
...
Blockly.Blocks['covid-behaviors_get'] = {
  init: function() {
    this.setColour(45);
    this.appendDummyInput('MAIN')
        .appendField("covid-behaviors.get")
        .appendField(new Blockly.FieldDropdown(covid-behaviors_PROPERTIES), "PROPERTY");
    this.appendDummyInput('SECOND')
        .appendField("filter")
        .appendField(new Blockly.FieldDropdown(covid-behaviors_INDEXES, function(option) {
  ...
  mutationToDom: function() {
    ...
    container.setAttribute('module', "covid-behaviors")
    return container;
  },
  ...
  updateShape_: function(index, index_value) {
    ...
    try {
        if (index != undefined && index != '(None)') {
            inputGroup.appendField(new Blockly.FieldDropdown(covid-behaviors_INDEX_VALUES[index]), 'INDEX_VALUE')
            if (index_value != undefined) {
                this.setFieldValue(index_value, 'INDEX_VALUE');
            } else {
                this.setFieldValue(covid-behaviors_INDEX_VALUES[index][0][0], 'INDEX_VALUE');
            }
        }
    } catch (e) {
...
Blockly.Python['covid-behaviors_get'] = function(block) {
    Blockly.Python.definitions_['import_covid-behaviors'] = 'import covid-behaviors';
    ...
    var code = 'covid-behaviors.get('+property+',' +index+','+index_value+')';
    return [code, Blockly.Python.ORDER_ATOMIC];
};

BlockMirrorTextToBlocks.prototype.MODULE_FUNCTION_SIGNATURES['covid-behaviors'] = {
    "get": {
        custom: function(node, parent) {
            ...
            return BlockMirrorTextToBlocks.create_block("covid-behaviors_get", null,
...
BlockMirrorBlockEditor.EXTRA_TOOLS['Data - covid-behaviors'] = '<category name="Data - Covid-Behaviors" colour="45">'+
                    '<block type="covid-behaviors_get"><mutation index="(None)" index_value=""></mutation></block>'+
                '</category>';